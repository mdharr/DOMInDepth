...
function createRegionDivs() {
    regions.forEach(region => {
        const name = region;
        const divElement = document.createElement('div');
        divElement.classList.add('region');
        divElement.classList.add(region);
...
function updateTimes() {
    ...
    timeZones.forEach(timeZone => {
        ...
        const pElement = document.getElementById(timeZone);
        if (pElement) {
            pElement.textContent = `${timeZone}: ${localTime}`;
        }
    });
}
...
function createTimezones() {
    ...
    timeZones.forEach(timeZone => {
        const pElement = document.createElement('p');
        pElement.classList.add('time-zone');
        pElement.setAttribute('id', timeZone);
        pElement.textContent = `${timeZone}: ${now.toLocaleString('en-US', { timeZone })}`;

        for (const region in regexMap) {
            if (regexMap[region].test(timeZone)) {
                const regionDiv = document.querySelector(`.${region}`);
                if (regionDiv) {
                    regionDiv.appendChild(pElement);
                }
                break;
            }
        }
...
function addToggleEventListeners() {
    const regionDivElements = document.querySelectorAll('.region');
    regionDivElements.forEach(divElement => {
        divElement.addEventListener('click', toggleDisplay);
    });
}

function toggleDisplay(event) {
    const regionDiv = event.currentTarget;
    const paragraphs = regionDiv.querySelectorAll('p');
